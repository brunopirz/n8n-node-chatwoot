...
export async function getChatwootBaseUrl(
	...
	const credentials = await this.getCredentials('singulHubChatwootApi');
...
export async function chatwootApiRequest(
	...
	try {
		return await this.helpers.httpRequestWithAuthentication.call(this, 'singulHubChatwootApi', options);
	} catch (error) {
